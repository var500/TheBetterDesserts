...
export default function WhatsAppWidget() {
  ...
  return (
    <div className="fixed right-6 bottom-6 z-50 flex cursor-pointer flex-col items-end md:right-10 md:bottom-10">
      ...
      >
        {/* If using your custom Text component, you can swap these tags: */}
        {/* <Text as="h3" variant="secondary" className="..."> */}
        <Text as={"h3"} className="mb-3 font-serif text-2xl text-[#2d3748]">
          Hi! How can we help you?
        </Text>

        <div className="mb-6 flex items-center text-sm text-gray-500">
          {/* Pulsing online indicator */}
          <Text as={"span"} className="relative mr-2 flex h-3 w-3">
            <Text
              as={"span"}
              className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#25D366] opacity-75"
            ></Text>
            <Text
              as={"span"}
              className="relative inline-flex h-3 w-3 rounded-full bg-[#25D366]"
            ></Text>
          </Text>
          Typically replies in a few minutes
        </div>

        {/* Actual Link to WhatsApp */}
        <a
          href={whatsappLink}
          target="_blank"
          rel="noreferrer"
          // We close the modal when they click the link
          onClick={() => setIsOpen(false)}
          className="flex w-full items-center justify-center rounded-xl bg-[#25D366] px-4 py-3 text-base font-bold text-white transition-colors hover:bg-[#20bd5a]"
        >
